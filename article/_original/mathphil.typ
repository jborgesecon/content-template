...
#let mathphil-article(
  ...
  set page(
    ...
    header: context {
      ...
      if page-num > 1 {
        ...
        if calc.odd(page-num) {
          // Recto: inner (left) short title, outer (right) page number
          grid(
            columns: (1fr, auto),
            align: (left + bottom, right + bottom),
            meta-label(display-short-title),
            meta-label(str(page-num)),
          )
        } else {
          // Verso: outer (left) page number, inner (right) short authors
          grid(
            columns: (auto, 1fr),
            align: (left + bottom, right + bottom),
            meta-label(str(page-num)),
            meta-label(display-short-authors),
          )
        }
      }
    },
    footer: context {
      ...
      if page-num == 1 {
        block(width: 100%)[
          #line(length: 100%, stroke: 0.4pt + jmprule)
          #v(2pt)
          #text(font: sans-font, size: 8.4pt, fill: jmpinkii)[
            #sym.copyright #year #copyright-text.
            #if doi != none and doi != "" [
              \ #link("https://doi.org/" + doi)[#text(fill: jmpblue)[https://doi.org/#doi]]
            ]
          ]
        ]
      }
  ...
  {
    text(size: 11pt, tracking: 0.04em)[#smallcaps[Journal of Mathematical Philosophy]]
    if issue != none and issue != "" {
      h(0.5em)
      text(font: sans-font, size: 8.1pt, tracking: 0.1em, fill: jmpinkii)[· #upper(issue)]
    }
    v(4.5pt)
    line(length: 100%, stroke: 0.8pt + jmprule)
    v(22pt)
    ...
    align(left)[
      ...
      #for author in authors [
        #block(spacing: 9pt)[
          #text(font: sans-font, size: 12.7pt, weight: "bold")[#author.name]
          #if "orcid" in author and author.orcid != none and author.orcid != "" [
            #h(0.3em)
            #link("https://orcid.org/" + author.orcid)[#text(size: 9pt, fill: rgb("#a6ce39"))[(iD)]]
          ]
          #v(1pt)
          #if "affiliation" in author [
            #text(size: 10.5pt, style: "italic", fill: jmpinkii)[#author.affiliation] \
          ]
          #if "email" in author [
            #text(font: sans-font, size: 10pt, fill: jmpinkii)[
              #link("mailto:" + author.email)[#text(fill: jmpblue)[#author.email]]
            ]
          ]
        ]
      ]
    ...
    if abstract != none [
      #align(left)[
        #text(font: sans-font, size: 8.1pt, weight: "bold", tracking: 0.14em)[ABSTRACT]
        #v(6.5pt)
        #block(width: 100%)[
          #set par(first-line-indent: 0pt, justify: true, leading: 0.65em)
          #text(size: 10.8pt)[#abstract]
        ]
      ]
      #if keywords.len() > 0 [
        #v(0.6em)
        #text(size: 9.5pt)[#text(weight: "bold")[Keywords:] #keywords.join(", ")]
      ]
      #if msc != none [
        #v(0.3em)
        #text(size: 9.5pt)[#text(weight: "bold")[MSC (2020):] #msc]
      ]
      #v(16.5pt)
    ]
  }
